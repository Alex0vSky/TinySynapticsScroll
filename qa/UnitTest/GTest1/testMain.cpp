...
int main(int argc, char **argv) {
	...
	if ( RunAllTestsDoesRunTests( ) ) {
		...
		try {
			void* lpWellKnownWinApiFunc = &::DestroyWindow;
			void* lpAnyNotZeroValue = (void*)1;
			void* oldFunc = 0;
			mockModule_patchModuleFunc( lpWellKnownWinApiFunc, lpAnyNotZeroValue, &oldFunc );
 			mockModule_restoreModuleFunc( lpWellKnownWinApiFunc, lpAnyNotZeroValue, &oldFunc );
		} catch (std::runtime_error) {
			std::cerr << strPref << "conflict detected! Run me second time to avoid conflict. Exit with error" << std::endl;
