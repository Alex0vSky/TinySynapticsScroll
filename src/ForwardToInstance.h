...
namespace PtsfCastSwitcher { 
	...
	static_assert( isDirectLParam< decltype( detail_::Unused_::bUnused_ ) >( ) || !isDirectLParam< void* >( ), "sizeof( void* ) cannot be equal sizeof( bool )" );
} // namespace PtsfCastSwitcher _
template <typename TPTMF, typename TPTSF, typename LAMBDA>
static LRESULT viaWindowLongPtr(HWND hWndMsg, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT uInitMsg, TPTMF VPTMF, TPTSF VPTSF, LAMBDA fnDefault) {
	using namespace Tool::Trait;
	using T = ptmf::class_t< TPTMF >;
	static_assert( std::is_base_of_v< ForwardToInstance::Base, T >, "class T must be inherited from 'ForwardToInstance::Base'" ); 
	T *self = nullptr;
	using TOFFSET = ptsf::field_t< TPTSF >;
	using TSTRUCT = ptsf::struct_t< TPTSF >;
...
protected:
	...
	{}
	template <typename TPTMF, typename TPTSF, typename LAMBDA>
	friend LRESULT detail_::viaWindowLongPtr(HWND hWndMsg, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT uInitMsg, TPTMF VPTMF, TPTSF VPTSF, LAMBDA fnDefault);
...
struct OfWindow {
	static Tool::Trait::ptmf::return_type_t< TPTMF > CALLBACK systemCallback(HWND hWndMsg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
